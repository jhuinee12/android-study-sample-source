...
public class BoardDBHelper extends SQLiteOpenHelper {
    ...
    public ArrayList<BoardData> rcyBoardSelect() {
        ...
        String selectQuery = String.format(" SELECT * FROM %s", BOARD_TABLE_NAME);
        boardCursor = sqlDb.rawQuery(selectQuery, null);
    ...
    public ArrayList<BoardData> rcyReaderSelect(String key_id) {
        ...
        // DB에 있는 데이터를 쉽게 처리하기 위해 Cursor를 사용하여 테이블에 있는 모든 데이터 출력
        String detailQuery = String.format(" SELECT * FROM %s where %s = '%s'", BOARD_TABLE_NAME, BOARD_KEY_ID, key_id);
        boardCursor = sqlDb.rawQuery(detailQuery, null);
        sqlDb.setTransactionSuccessful();
    ...
    public ArrayList<BoardData> rcyBoardSearch(String searchtext) {
        rcyBoardList.clear();
        ...
        // DB에 있는 데이터를 쉽게 처리하기 위해 Cursor를 사용하여 테이블에 있는 모든 데이터 출력
        String searchQuery = String.format(" SELECT * FROM %s where %s like '%%%s%%'", BOARD_TABLE_NAME, BOARD_KEY_TITLE, searchtext);
        boardCursor = sqlDb.rawQuery(searchQuery, null);
        sqlDb.setTransactionSuccessful();
